...
object Lenses {
  ...
  val l1: Lens[(String, Int), String] = Lens[(String, Int), String](_._1)(s => _ => (s, 0))   // Fails GetPut

  // Complete the second example from page 6, and the example from page 7 below:

  // page 6 in Foster et al.:
  val l2: Lens[String, (String, Int)] = Lens[String, (String, Int)]((_, 0))(si => _ => si._1) // Fails PutGet

  // page 7 in Foster et al.
  val l3: Lens[(String, Int), String] = Lens[(String, Int), String](_._1)(s => sn =>          // Fails PutPut
    if (s == sn._1) (s, sn._2) else (s, sn._2 + 1)
  )
  ...
  def codiag[A]: Lens[A \/ A, A] = Lens.codiagonal
  ...
  def codiag1[A]: Lens[A \/ A, A] = Lens.codiagonal
  ...
  case class Address(zipcode: ZipCode, country: String)
  case class University(students: Students, address: Address)
  ...
  val _country: Lens[Address, String] = Lens[Address, String](_.country)(s => a => a.copy(country = s))
  ...
  // Exercise 7. Use filterIndex(p) to only capitalize country names of the
  ...
  val itu4: University =
    (_students ^|->> filterIndex((x: String) => x.startsWith("A")) ^|-> _country).modify(_.toUpperCase)(itu)
  ...
  def setIth[A](n: Integer): Optional[List[A], A] =
    Optional[List[A], A](_.lift(n - 1))(a => la => if (la.size < n) la else la.updated(n - 1, a))
  ...
  def setIth1[A](n: Integer, default: A): Lens[List[A], A] =
    Lens[List[A], A](_.lift(n - 1).orElse(Some(default)).get)(a => l =>
      if (l.size < n) (l ++ List.fill(n - l.size)(default)).updated(n - 1, a)
      else l.updated(n - 1, a)
    )
  ...
  // "imperative" programming, by making a structure updatable, even deeply.
  // For a simple example, use sethIth below to increment the third element on a
  // list list0
  import scala.collection.immutable.Set
  val list0: List[Int] = List(1, 2, 3, 4, 5, 6)
  val list1: List[Int] = setIth(3).modify((x: Int) => x + 1)(list0)
  val set0: Set[Int] = Set(1,2,3,4,5)

  def contains(k: Int): Lens[Set[Int], Boolean] = Lens[Set[Int], Boolean](_ contains k)(b => li => (b, li) match {
    case (true, s) => s + k
    case (false, s) => s - k
  })

  // Consider these
  // contains(9).set(true)(set0)
  // contains(9).set(false)(set0)
  // contains(9).get(set0)
