...
class exam2017Spec extends FreeSpec with Checkers {
  ...
  "QUESTION 1" - {
    ...
    "simple reduceByKey sccenario test" in {
      ...
      reduceByKey[Int, Int](l) {
        _ + _
      } shouldBe List((1, 4), (2, 100), (3, 7))
    }
  ...
  "QUESTION 4" - {
    ...
    "TASK 7. Transition step" - {
      ...
      "empty machine" in check {
        forAll(EmptyMachine) { m => step(Coin)(m) == m }
      }
    ...
    "TASK 8. Simulation" - {
      ...
      implicit val arbInput: Arbitrary[Input] = Arbitrary[Input](oneOf(Coin, Brew))

      "the two solutions are equivalent" in check {
        forAll(genM) { ini =>
          forAll { inputs: List[Input] =>
            simulateMachine(ini)(inputs) == simulateMachine2(ini)(inputs)
          }
        }
      }
  ...
  "QUESTION 5. Task 9" - {
    ...
    import handIns.stream.Exercise4.Stream
    import exams.exam2017.Q5._
    ...
    "empty" in {
      flatten(Stream()).toList shouldBe Nil
    }
  ...
  "QUESTION 7" - {
    ...
    import exams.exam2017.Q7


    val t = List(1, 2, 3).foldLeft[FingerTree[Int]](Empty()) { case (tree, b) => tree addR b }
  ...
  "QUESTION 8" - {
    ...
    "nullOption get on int" in check {
      forAll { n: Int => nullOption[Integer].get(n).contains(n) }
    }
    ...
    "nullOption put-get" in check {
      PutGet[String, Option[String]](nullOption)
    }
    "nullOption get-put" in check {
      GetPut[String, Option[String]](nullOption)
    }
    "nullOption put-put" in check {
      PutPut[String, Option[String]](nullOption)
    }
