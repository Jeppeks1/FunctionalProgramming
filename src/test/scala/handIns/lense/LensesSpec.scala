...
import handIns.lense.Lenses._


class LensesSpec extends FlatSpec with Checkers with Matchers {
  ...
  def PutGet[C, A](l: Lens[C, A])
                  (implicit aA: Arbitrary[A], aC: Arbitrary[C]): Prop = forAll { (a: A, c: C) =>
    l.get(l.set(a)(c)) == a
  }

  def GetPut[C, A](l: Lens[C, A])
                  (implicit aA: Arbitrary[A], aC: Arbitrary[C]): Prop = forAll { c: C =>
    l.set(l.get(c))(c) == c
  }

  def PutPut[C, A](l: Lens[C, A])
                  (implicit aA: Arbitrary[A], aC: Arbitrary[C]): Prop = forAll { (a: A, aa: A, c: C) =>
    l.set(a)(l.set(aa)(c)) == l.set(a)(c)
  }

  // specification of a total lense laws (refers to the laws above)
  def wellBehavedTotalLense[A, C](l: Lens[C, A])
                                 (implicit ac: Arbitrary[C], aa: Arbitrary[A]): Unit = {
    it should "obey the PutGet law" in check {
      PutGet(l)
    }
    it should "obey the GetPut law" in check {
      GetPut(l)
    }
  }
  ...
  def veryWellBehavedTotalLense[A, C](l: Lens[C, A])
    ...
    it should "obey the PutPut law" in check {
      PutPut(l)
    }
  }
  ...
  "l3" should behave like veryWellBehavedTotalLense(l3)
  "nullOption" should behave like veryWellBehavedTotalLense(nullOption[Int])
  "eitherOption" should behave like veryWellBehavedTotalLense(eitherOption[String, Int]("Fail"))
