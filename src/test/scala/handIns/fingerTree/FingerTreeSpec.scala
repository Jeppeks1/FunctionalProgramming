...
class FingerTreeSpec extends FlatSpec with Checkers with Matchers {
  ...
  //
  implicit def arbFingerTree[A](implicit arb: Arbitrary[A]): Arbitrary[FingerTree[A]] =
  ...
  it should "be an identitity on trees" in check {
    forAll(fingerTreeOfN(100, Gen.choose[Int](0, 1000))) {
      t: FingerTree[Int] => FingerTree.toTree(t) == t
    }
  }
  ...
  it should "be ConsL(_,ConsL(_,_)) on any tree larger than 3" in check {
    val ft3plus = Gen.choose(3, 100) flatMap {
      fingerTreeOfN(_, arbitrary[Int])
    }
    forAll(ft3plus) { t: FingerTree[Int] =>
      t match {
        case ConsL(_, ConsL(_, _)) => true
        case _ => false
      }
    }
  }
  ...
  it should "have the right prefix on any tree larger than 3" in check {
    val list3plus = Gen.choose(3, 100) flatMap {
      Gen.listOfN(_, arbitrary[Int])
    }
